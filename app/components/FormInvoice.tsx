...
export function FormCreateOrEditInvoice({
	...
	return (
		<Dialog.Root open={isFormOpen} onOpenChange={handleCloseForm}>
			...
			<Dialog.Portal>
				<Dialog.Overlay className="fixed inset-0 animate-fade-in bg-overlay px-6 transition-all duration-300 md:h-screen md:px-0" />
				<Dialog.Content className="fixed bottom-0 left-0 h-full w-full overflow-y-auto bg-white p-6 pt-24 transition-all duration-300 ease-linear scrollbar scrollbar-track-selago scrollbar-thumb-shipCove scrollbar-thumb-rounded-md scrollbar-w-3 data-[state=closed]:-translate-x-full data-[state=open]:translate-x-0 dark:bg-mirage2 dark:scrollbar-track-mirage2 dark:scrollbar-thumb-ebony md:max-w-3xl md:pt-24 lg:max-w-4xl lg:rounded-r-2xl lg:pl-[7.625rem] lg:pr-11 lg:pt-6">
					...
					<form onSubmit={handleSubmit(onSubmit)} className="relative">
						<fieldset className="space-between-inputs">
							...
							<div className="mb-6 flex flex-wrap gap-3 [&_label:not(:last-child)]:flex-1 md:[&_label]:min-w-fit md:[&_label]:flex-1">
								<LabelTextField
									label="City"
									isRequiredField
						...
						<div
							className={clsx(
								"absolute -left-6 flex w-[calc(100%_+_48px)] flex-wrap-reverse items-center justify-center gap-3 bg-transparent p-6 dark:bg-mirage2 md:static md:w-auto md:bg-transparent md:p-[1.5rem_1.5rem_0] md:px-0",
								{
							...
							<div className="space-x-3">
								{isEditing ? (
									...
									<button
										...
										className="h-12 rounded-3xl bg-otherDark px-6 text-xs font-bold text-white transition-all first-letter:capitalize disabled:cursor-not-allowed disabled:opacity-50 hover:opacity-90"
								...
								<button
									...
									className="h-12 rounded-3xl bg-purple px-6 text-xs font-bold text-white transition-all first-letter:capitalize disabled:cursor-not-allowed disabled:opacity-50 hover:bg-heliotrope"
